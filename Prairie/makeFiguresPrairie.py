...
summaryfolder = '/data/JData/A/A57_GtACR_Mi1/A57_Data/PrairieSummary'
...
for row, dseries in maskfiles.iterrows():
    ...
    ## need to load either 
    image = cc2.read_TimeSeries_HDF5(dseries['Image_files']).getIntensityData()
    stdImg = np.std(image[:, :, :, :, 0], axis =0).squeeze()
    stdImg = stdImg.T
    ...
    MIP= np.max(image[:, :, :, :, 0], axis =0).squeeze()
    MIP= MIP.T
