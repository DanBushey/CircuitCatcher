...
class ImageWindow3(ImageView):
    def __init__(self, *args, **kargs):
        ...
        if 'title' in kargs:
            self.win.setWindowTitle(kargs['title'])
            del kargs['title']
        tsDB.TimeSeriesView.__init__(self, self.win)
        ####################
        '''
        Does not work because not call mouse movements??
        Need a simpler graph than TimeSeriesView??
        '''
        ####################
        if len(args) > 0 or len(kargs) > 0:
            self.setImage(*args, **kargs)
