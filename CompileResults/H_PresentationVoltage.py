...
bad = exceldata[exceldata['Grade'] != 1]
print(bad)
...
for ckey in transgene.keys():
    if ckey not in exceldata['Genotype'].unique():
        listToDrop.append(ckey)
        #newdict.pop(ckey, None)
print(len(newdict.keys()))
print(len(exceldata['Genotype'].unique()))
transgene = newdict

### get counts for M8-10 roi region only
excelM810 = exceldata[exceldata['roi'].str.contains('M8-10')]
groups = excelM810.groupby(['Genotype'])
groups[['Sample Name']].count().to_excel(os.path.join(saveFig, 'CountsAll2.xlsx'))
table = groups[['Sample Name']].count()
table['Kd'] = ''
for cgene in table.index:
    table['Kd'].loc[cgene] = transgene[cgene]['kd']
table = table.sort_values(by = ['Kd'])
table.to_excel(os.path.join(saveFig, 'CountsAll3.xlsx'))
...
for croi in roiList:
    ...
    for cgen in transgene.keys():
        ...
        ax1.plot(timeStamp, np.mean(intensitydata, axis=1), color = transgene[cgen]['color'], linewidth=2)
    ax1.set_xlim([0, timeStamp[-1]])
    fig1.savefig(os.path.join(targetdirectory, croi + '_Mean_rawtrace.jpeg'))
    plt.close(fig1)
...
for croi in roiList:
    ...
    for cgen in transgene.keys():
        ...
        ax1.plot(timeStamp, np.mean(intensitydata, axis=1), color = transgene[cgen]['color'], linewidth=2)
    ax1.set_xlim([0, timeStamp[-1]])
    fig1.savefig(os.path.join(targetdirectory, croi + '_Mean_Normalized_trace.jpeg'))
    plt.close(fig1)
...
for crois in roiList:
    for cstimindex, cstim in enumerate(stimPeriods):
        ...
        for cgen in transgene.keys():
            cdataframe = cc2.selectInFrame(exceldata, {'roi' : crois, 'Genotype' : cgen})
...
#functionName = ['Max', 'Mean', 'Median', 'SNR', 'delay2Max']
#columns1 = ['Max_Absolute_dFF', 'Mean_dFF', 'Median_dFF', 'Signal2Noise', 'Delay_Peak_Amplitude', 'Min_Intensity_After', 'Decay_After'] # columngs for the dataframe holding data
statsDict = {'Max' : 'Max_Absolute_dF/F', 
             'Mean': 'Mean_dF/F',
             'Median': 'Median_dF/F', 
             'SNR': 'Signal_to_Noise', 
             'delay2Max':'Delay_Peak_Amplitude',
             'baseline_SD' : 'Baseline_Standard_Deviation_dF/F',
...
for cstat in statsDict.keys():
    ...
    for crois in roiList:
        ...
        ax1 = fig1.add_axes([0.07, 0.15, 0.4, 0.8])
